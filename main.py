...
ws_vertex.connect(config['vertex'][env]['blast']['subscriptions'], header=["Sec-WebSocket-Extensions: permessage-deflate"])
...
Vertex = Product('Vertex', ws_vertex, config['vertex'][env]['blast']['gateway_rest'], 2, 1)
...
def main():
    ...
    while i <= 10:
        ...
        print(f"Time taken to fetch Vertex data: {end-start}")
        start = timeit.default_timer()
        raw_data_drift = apc.get_market_liquidity_drift(Drift)
        end = timeit.default_timer()
        print(f"Time taken to fetch Drift data: {end - start}")
